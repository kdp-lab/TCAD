...
void cleanup(FILE *gridfp, float **xvtx, float **evtx, int num_arrays)
{
   // Close the grid file if open
   if (gridfp != NULL)
   {
      fclose(gridfp);
   }

   // Free memory for xvtx
   if (xvtx != NULL)
   {
...
/* find and read in the grid coordinates */
       ...
       rewind(gridfp); // Ensure the file pointer is at the beginning

       // Debug each line being read
       char line[256];
       int vertices_found = 0;

       while (fgets(line, sizeof(line), gridfp))
       {
          ...
          if (strstr(line, "Vertices") != NULL)
          {
             ...
             if (sscanf(line, " Vertices (%d)", &nvtx) != 1)
             {
                printf("Error: Unable to parse number of vertices from line: %s", line);
                fclose(gridfp);
                return 1;
             }

             printf("Found Vertices: number of vertices = %d\n", nvtx);

             // Allocate memory for xvtx and evtx after parsing nvtx
             // Parse the number of vertices and allocate memory
             if (sscanf(line, " Vertices (%d)", &nvtx) != 1)
             {
                printf("Error: Unable to parse number of vertices from line: %s", line);
                cleanup(gridfp, NULL, NULL, 0); // Cleanup and exit
                return 1;
             }
             printf("Found Vertices: number of vertices = %d\n", nvtx);

             // Allocate memory for xvtx
             xvtx = malloc(3 * sizeof(float *));
             if (xvtx == NULL)
             {
                printf("Error: Unable to allocate memory for xvtx.\n");
                cleanup(gridfp, NULL, NULL, 0); // Cleanup and exit
                return 1;
             }
             for (int i = 0; i < 3; i++)
             {
                xvtx[i] = malloc(nvtx * sizeof(float));
                if (xvtx[i] == NULL)
                {
                   printf("Error: Unable to allocate memory for xvtx[%d].\n", i);
                   cleanup(gridfp, xvtx, NULL, i); // Cleanup partially allocated arrays
                   return 1;
                }
             }

             // Allocate memory for evtx
             evtx = malloc(3 * sizeof(float *));
             if (evtx == NULL)
             {
                printf("Error: Unable to allocate memory for evtx.\n");
                cleanup(gridfp, xvtx, NULL, 3); // Cleanup and exit
                return 1;
             }
             for (int i = 0; i < 3; i++)
             {
                evtx[i] = malloc(nvtx * sizeof(float));
                if (evtx[i] == NULL)
                {
                   printf("Error: Unable to allocate memory for evtx[%d].\n", i);
                   cleanup(gridfp, xvtx, evtx, i); // Cleanup partially allocated arrays
                   return 1;
                }
             }

             break; // Exit the loop after finding "Vertices"
          }
       }

       // If "Vertices" section not found, exit with an error
       if (!vertices_found)
       {
          printf("Error: 'Vertices' section not found in the grid file.\n");
          fclose(gridfp);
          return 1;
       }
       ...
       for (i = 0; i < nvtx; ++i)
       {
          if (fscanf(gridfp, "%f %f %f", &xvtx[2][i], &xvtx[0][i], &xvtx[1][i]) != 3)
          {
...
/* find and read in the electric field at each vertex */
       ...
       while (fscanf(desfp,"%s", inp1) !=EOF) {
         if(strcmp(inp1,"(\"ElectricField\")") == 0) {
           while (fscanf(desfp,"%s", inp1) !=EOF) {
