...
int main(int argc, char *argv[])
{
    ...
    static float xvtx[3][3000000], evtx[3][3000000];
...
/* find and read in the grid coordinates */
       ...
       while (fscanf(gridfp, "%s", inp1) != EOF)
       {
          if (strcmp(inp1, "Vertices") == 0)
          {
             ...
             if (nvtx > 3000000)
             {
                printf("Error: Too many vertices (%d). Increase array size or use dynamic allocation.\n", nvtx);
                return 1;
             }
             for (i = 0; i < nvtx; ++i)
             {
       ...
       printf("Error: 'Vertices' keyword not found in the grid file.\n");
       return 1;
